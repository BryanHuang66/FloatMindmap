...
import settingWebViewControllerInst from "settingWebViewController/main"
...
const SettingWebViewController = JSB.defineClass(
  getObjCClassDeclar("SettingWebViewController", "UITableViewController"),
settingWebViewControllerInst
)
...
const sceneWillConnect = () => {
  ...
  self.webController = new SettingWebViewController();
  self.webController.mainPath = self.mainPath 
  self.layoutViewController = function () {
    var frame = Application.sharedInstance().studyController(self.window)
      .view.bounds;

    if (self.webController.position == "lt")
    ...
    else if (self.webController.position == "rt")
      self.webController.view.frame = {
        x: frame.width - 525,
        y: 70,
        width: 480,
    ...
    else if (self.webController.position == "ct")
      self.webController.view.frame = {
        x: (frame.width - 480) / 2,
        y: (frame.height - 375) / 2,
        width: 480,
        height: 375,
      };
    else if (self.webController.position == "lb")
      self.webController.view.frame = {
        x: 0,
        y: frame.height - 375,
        width: 480,
    ...
    else if (self.webController.position == "rb")
      self.webController.view.frame = {
        x: frame.width - 480,
        y: frame.height - 375,
        width: 480,
...
const sceneDidDisconnect = () => {
  log("关闭窗口", "lifeCycle")
  // 只要打开过文档，再关闭窗口就保存
  profile.autodetect.visible = false
